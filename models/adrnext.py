...
class _AttentionModule(nn.Module):
    """
    """
    def __init__(self, inplanes_high, inplanes_low, planes, norm_layer=None):
        super(_AttentionModule, self).__init__()
        self.w_y = nn.Sequential(
                nn.Conv2d(inplanes_low, planes, kernel_size=1, stride=1, padding=0, bias=True),
                norm_layer(planes),
                nn.ReLU(inplace=True)
                )
        
        self.w_x = nn.Sequential(
                nn.Conv2d(inplanes_high, planes, kernel_size=1, stride=1, padding=0, bias=True),
                norm_layer(planes),
                nn.ReLU(inplace=True)
                )
        
        self.psi = nn.Sequential(
                nn.Conv2d(planes, 1, kernel_size=1, stride=1, padding=0, bias=True),
                norm_layer(1),
                nn.Sigmoid()
                )
...
class ADRNeXt(nn.Module):
    ...
    ***
    ...
    def __init__(self, block, attentionblock, layers, num_classes, norm_layer=None, groups=32, dilate_scale=8):
        ...
        self.attention = self._make_attention_layer(_AttentionModule, 512, 128, planes=64, norm_layer=norm_layer)
        
        self.one_skip = nn.Sequential(
        ...
        self.conv_last = nn.Sequential(
                nn.Conv2d(512 + 128 + 64, 512, kernel_size=3, padding=1, bias=False),
                norm_layer(512),
                nn.ReLU(inplace=True),
                nn.Dropout2d(0.1),
    ...
    def _make_attention_layer(self, block, inplanes_high, inplanes_low, planes, norm_layer):
        return block(inplanes_high, inplanes_low, planes, norm_layer)
    ...
    def forward(self, x):  # (b, 3, w, h)
        ...
        if not (x2_size[2] == x0_size[2] and x2_size[3] == x0_size[3]):
            x2 = nn.functional.upsample(x2, size=(x0_size[2], x0_size[3]), mode='bilinear')
        assert x2.size()[2:3] == x0.size()[2:3], "{0} vs {1}".format(x2.size(), x0.size())
        ...
        # Upsample x4
        x3_size = x3.size()
        if not (x3_size[2] == x_size[2] and x3_size[3] == x_size[3]):
            x3 = nn.functional.upsample(x3, size=(x_size[2], x_size[3]), mode='bilinear')
        assert x3.size()[2:3] == x_ori.size()[2:3], "{0} vs {1}".format(x3.size(), x_ori.size())

...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  5 12:47:15 2019
...
__all__ = ['PASPNet', 'PASPNet50', 'PASPNet101', 'PASPNet152']
...
class _ASPPModule_v2at(nn.Module):
    """"
    : param in_planes: default = 2048
    : param out_planes: default = 256
    : param os: 8 0r 16
    """
    def __init__(self, in_planes=2048, out_planes=256, os=8, scale=3, reduction=1, norm_layer=None):
        super(_ASPPModule_v2at, self).__init__()
        if os == 8:
            rates = [3, 2, 6, 12, 18]   # [1, 1, 6, 12, 18]
        elif os == 16:
            rates = [3, 1, 12, 24, 36]  # [1, 1, 12, 24, 36]
...
class ChannelsAttentionModule(nn.Module):
    ...
    """
    def __init__(self, inplanes_low=512, inplanes_high=512, planes=256, outplanes=256,scale=3, reduction=1, norm_layer=None):  # reduction=1, 4, 8, 16
        super(ChannelsAttentionModule, self).__init__()
        
        self.channelsdown = nn.Sequential(
                nn.Conv2d(in_channels=inplanes_low, out_channels=inplanes_high,
        ...
        self.channelsattention = nn.Sequential(
                ...
                nn.Conv2d(in_channels=inplanes_high, out_channels=int(inplanes_high // reduction), 
                          kernel_size=scale, stride=1, padding=0, dilation=1,
                          groups=int(inplanes_high // reduction), bias=False),
                # norm_layer(inplanes_high, momentum=0.95),
                nn.ReLU(inplace=True),
        ...
        self.fusion = nn.Sequential(
                nn.Conv2d(in_channels=inplanes_high, out_channels=planes, 
                          kernel_size=3, stride=1, padding=1, dilation=1, bias=False),
                norm_layer(planes),
                nn.ReLU(inplace=True) # nn.Softmax(dim=1)
                )
    ...
    def forward(self, x_low, x_high):
        x_low = self.channelsdown(x_low)
        # ca = self.channelsattention(x_high)
        
        xl_size = x_low.size()
        xh_size = x_high.size()
        if not (xh_size[2] == xl_size[2] and xh_size[3] == xl_size[3]):
            x_high = F.interpolate(x_high, size=(xl_size[2], xl_size[3]), mode='bilinear', align_corners=True) # umsample
        
        assert x_low.size()[1:3] == x_high.size()[1:3], "{0} vs {1}".format(x_low.size(), x_high.size())

        ca = self.channelsattention(x_high)
        x_high = x_high + x_low * ca
...
class ChannelsAttentionModule_v2(nn.Module):
    ...
    Channel Attention Module : C: 256 512 1024 2048
    """
    def __init__(self, inplanes_low=512, inplanes_high=512, planes=256, outplanes=512, scale=3, reduction=1, norm_layer=None):  # reduction=1, 4, 8, 16
        ...
        self.channelsattention = nn.Sequential(
                nn.AdaptiveAvgPool2d(scale),
                nn.Conv2d(in_channels=inplanes_high + planes, out_channels=int((inplanes_high + planes ) // reduction), 
                          kernel_size=scale, stride=1, padding=0, dilation=1,
                          groups=int((inplanes_high + planes ) // reduction), bias=False),
                # norm_layer(inplanes_high, momentum=0.95),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=int((inplanes_high + planes ) // reduction), out_channels=planes, 
                          kernel_size=1,stride=1, padding=0, dilation=1, bias=False),
                nn.Sigmoid() # nn.Softmax(dim=1)
                )
                
        self.fusion = nn.Sequential(
                nn.Conv2d(in_channels=inplanes_high +planes, out_channels=outplanes, 
                          kernel_size=3, stride=1, padding=1, dilation=1, bias=False),
                norm_layer(planes),
                nn.ReLU(inplace=True) # nn.Softmax(dim=1)
                )
    ...
    def forward(self, x_low, x_high):
        ...
        assert x_low.size()[1:3] == x_high.size()[1:3], "{0} vs {1}".format(x_low.size(), x_high.size())
        ca =torch.cat([x_low, x_high], dim=1)
        ca = self.channelsattention(ca)
        x_high = x_high + x_low * ca
...
class ChannelsAttentionModule_v3(nn.Module):
    ...
    Channel Attention Module : C: 256 512 1024 2048
    """
    def __init__(self, inplanes_low=2048, inplanes_high=512, planes=256, outplanes=512, scale=3, reduction=1, norm_layer=None):  # reduction=1, 4, 8, 16
        ...
        self.channelsattention = nn.Sequential(
                ...
                # norm_layer(inplanes_high, momentum=0.95),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=int((inplanes_high + planes) // reduction), out_channels=inplanes_high + planes, 
                          kernel_size=1,stride=1, padding=0, dilation=1, bias=False),
                nn.Sigmoid() # nn.Softmax(dim=1)
                )
...
class PASPNet(nn.Module):
    ...
    ***
    ...
    def __init__(self, block, layers, num_classes, norm_layer=None, dilate_scale=16):
        ...
        self.layer5 = self._make_asppat_layer(_ASPPModule_v2at, 2048, 256, os=8, scale=3, reduction=1, norm_layer=norm_layer)
        
        # self.ca52 = self._make_channelattention_layer(ChannelsAttentionModule, 512, 512, 256, 5, 1, norm_layer=norm_layer)
        # self.ca21 = self._make_channelattention_layer(ChannelsAttentionModule, 256, 256, 256, 3, 1, norm_layer=norm_layer)
        
        self.ca54 = self._make_channelattention_layer(ChannelsAttentionModule_v3, 2048, 512, 256, 512, 3, 1, norm_layer=norm_layer)
        self.ca43 = self._make_channelattention_layer(ChannelsAttentionModule_v3, 1024, 512, 128, 512, 3, 1, norm_layer=norm_layer)
        self.ca32 = self._make_channelattention_layer(ChannelsAttentionModule_v3, 512, 512, 64, 512, 3, 1, norm_layer=norm_layer)
        self.ca21 = self._make_channelattention_layer(ChannelsAttentionModule_v3, 256, 512, 32, 512, 3, 1, norm_layer=norm_layer)
    ...
    def _make_asppat_layer(self, block, inplanes, planes, os, scale, reduction, norm_layer):
    ...
    def _make_channelattention_layer(self, block, inplanes_low, inplanes_high, planes, outplanes, scale, reduction, norm_layer):
        return block(inplanes_low, inplanes_high, planes, outplanes, scale, reduction, norm_layer)
    ...
    def forward(self, x):
        ...
        x5 = self.layer5(x4)   # [b, 512, x/16, x/16]
        
        ca = self.ca54(x4, x5) # [b, 256, x/16, x/16]
        ca = self.ca43(x3, ca) # [b, 256, x/16, x/16]
        ca = self.ca32(x2, ca) # [b, 256, x/8, x/8]
        ca = self.ca21(x1, ca) # [b, 256, x/4, x/4]
        
        
        x_last = self.conv_last(ca) # [b, 150, x/4, x/4]

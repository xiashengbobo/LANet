...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 10:13:11 2019
...
class CASPNet(nn.Module):
    ...
    ***
    ...
    def forward(self, x):
        ...
        x5_size = x5.size()
        x_size = x.size()
        if not (x5_size[2] == x_size[2] and x5_size[3] == x_size[3]):
            x5 = F.interpolate(x5, size=(x_size[2], x_size[3]), mode='bilinear',  align_corners=True) # umsample
            # x3 = nn.functional.upsample(x3, size=(x_size[2], x_size[3]), mode='bilinear') 
        assert x5.size()[2:3] == x.size()[2:3], "{0} vs {1}".format(x5.size(), x.size())
        
        x_last = torch.cat([x, x5], dim=1)
...
class CASPNet_v2(nn.Module):
    ...
    ***
    ...
    def __init__(self, block, layers, num_classes, norm_layer=None, dilate_scale=16):
        ...
        self.aspp = self._make_asppca_layer(_ASPPModule_v2M, 2048, 768, os=8, scale=3, reduction=1, norm_layer=norm_layer)
        
        self.ds1 = self._make_downsample_layer(DownSample4Module, 256, 64, norm_layer=norm_layer)
        self.ds2 = self._make_downsample_layer(DownSample2Module, 512, 128, norm_layer=norm_layer)
        self.ds3 = self._make_downsample_layer(DownSample1Module, 1024, 256, norm_layer=norm_layer)
       
        self.conv_last = nn.Sequential(
                nn.Conv2d(in_channels=768, out_channels=512, kernel_size=3, stride=1, padding=1, dilation=1, bias=False),
                norm_layer(512),
                nn.ReLU(inplace=True), 
                nn.Dropout2d(0.2),
                nn.Conv2d(512, num_classes, kernel_size=1)
    ...
    def _make_asppca_layer(self, block, inplanes, planes, os, scale, reduction, norm_layer):
        return block(inplanes, planes, os, scale, reduction, norm_layer)
    
    def _make_downsample_layer(self, block, inplanes, planes, norm_layer):
        return block(inplanes, planes, norm_layer)
    ...
    def forward(self, x):
        ...
        x_last = self.aspp(x4, x3d, x2d, x1d) # [b, 1728, x/16, x/16]

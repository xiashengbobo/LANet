...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  5 19:33:44 2019
...
class AttentionBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = original_out + original_out * out
...
class AttentionBottleneck_v2(nn.Module):
    """
    ResNet Bottleneck
    """
    expansion = 4
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, norm_layer=None):
        super(AttentionBottleneck_v2, self).__init__()
        ...
        self.subAvgpool1 = nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=(3,3)),  # GlobalAvgPool 3, 5, 7
        ...
        self.subAvgpool2 = nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=(3,3)),  # GlobalAvgPool 3, 5, 7
                nn.Conv2d(in_channels=planes, out_channels=planes, 
                          kernel_size=3, stride=1, padding=0, dilation=1, 
                          groups=planes, bias=False),
        ...
        self.subAvgpool3 = nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=(3,3)),  # GlobalAvgPool 3, 5, 7
                nn.Conv2d(in_channels=planes * 4, out_channels=planes * 4, 
                          kernel_size=3, stride=1, padding=0, dilation=1, 
                          groups=planes, bias=False),
        ...
        self.ca = nn.Sequential(
                nn.Conv2d(in_channels=planes * 6, out_channels=planes * 4, 
                          kernel_size=1, stride=1, padding=0, dilation=1, bias=True),
                nn.Sigmoid() 
                ) 
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        ca1 = self.subAvgpool1(out)
        out = self.relu(out)
        
        out = self.conv2(out)
        out = self.bn2(out)
        ca2 = self.subAvgpool2(out)
        out = self.relu(out)
        ...
        # attention
        original_out = out
        ca3 = self.subAvgpool3(out)
        cat = torch.cat([ca1, ca2, ca3], dim=1)
        cat = self.ca(cat)
        out = out + out * cat
...
class _ASPPModule_ATv2(nn.Module):
    ...
    def __init__(self, in_planes=2048, out_planes=256, os=8, reduction=1, norm_layer=None):
        ...
        self.aspp_catdown = nn.Sequential(
                ...
                norm_layer(2 * out_planes),  #  256 * 2
                nn.ReLU(inplace=True),
                nn.Conv2d(2 * out_planes, out_planes, kernel_size=1, bias=True)
                )
...
class ChannelsAttentionModule(nn.Module):
    ...
    """
    def __init__(self, planes=512, scale=1, reduction=1, norm_layer=None):  # reduction=1, 4, 8, 16
        ...
        self.channelsattention = nn.Sequential(
                ...
                nn.Conv2d(in_channels=planes, out_channels=int(planes // reduction), 
                          ...
                          groups=int(planes // reduction), bias=False),
    ...
    def forward(self, x_low, x_high):
        x_psi = self.psi(x_low)
        x_low += x_low * x_psi
        
        assert x_low.size()[0:3] == x_high.size()[0:3], "{0} vs {1}".format(x_low.size(), x_high.size())
...
class PASPNet(nn.Module):
    ...
    ***
    ...
    def __init__(self, block, attentionblock, layers, num_classes, norm_layer=None, dilate_scale=16):
        self.inplanes = 64
        ...
        if dilate_scale == 8:
            ...
            self.layer4 = self._make_layer(attentionblock, 512, layers[3], stride=1,
            ...
            self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                           norm_layer=norm_layer)
            self.layer4 = self._make_layer(attentionblock, 512, layers[3], stride=2,
                                           norm_layer=norm_layer)
...
############################################################################   
def ACSPNet50(num_classes=150, norm_layer=nn.BatchNorm2d, **kwargs):
    model = ACSPNet(Bottleneck, AttentionBottleneck, [3, 4, 6, 3], num_classes, norm_layer, **kwargs)
    return model

def ACSPNet101(num_classes=150, norm_layer=nn.BatchNorm2d, **kwargs):
    model = ACSPNet(Bottleneck, AttentionBottleneck, [3, 4, 23, 3], num_classes, norm_layer, **kwargs)
    return model

def ACSPNet152(num_classes=150, norm_layer=nn.BatchNorm2d, **kwargs):
    model = ACSPNet(Bottleneck, AttentionBottleneck, [3, 8, 36, 3], num_classes, norm_layer, **kwargs)
    return model
